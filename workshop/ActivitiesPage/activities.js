...
const option = document.getElementById("activityList");
const activititySelectList = document.getElementById("activitySelectList");
const infoButton = document.getElementById("infoButton");

function loadCatagoryList() {
...
for (const activity of activities) {
  ...
  activititySelectList.appendChild(option);
}
function displayActivityInfo() {
  let activityId = activititySelectList.value;
  for (const activity of activities) {
    if (activity.id == activityId) {
      const display = ` ${activity.name}
                                 Description :${activity.description}
                                Cost: ${activity.price}`;
      activityInfo.innerText = display;
    }
  }
}
...
loadActivityList();
infoButton.onclick = displayActivityInfo;
